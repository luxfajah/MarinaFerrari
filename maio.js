...
function addCommentByText(id, text, showT=true) {
  ...
  const displayName = (currentUser === 'marina' || currentUser === 'marina@duasmaos.com.br') ? 'Marina Ferrari' : 'Duas Mãos (Admin)';
...
function populateLbRight(postId){
  ...
  if(cap) {
    cap.innerHTML=`<strong>marinaferrari</strong> ${meta.caption||''}`;
  }
